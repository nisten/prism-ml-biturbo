...
// ---------------------------------------------------------------------------
// TBQ4_0 → F32 dequantize on CUDA using Fast Walsh-Hadamard Transform (FWHT)
//
// block_tbq4_0: uint8_t qs[128] (nibble-packed, 256 values), ggml_half d
// Dequant: nibble → codebook*scale_down → FWHT inverse rotation → * norm
// Two independent 128-element sub-blocks per block.
// FWHT inverse: x = D_s1 · H_norm · D_s2 · y  (apply s2, butterfly, scale, apply s1)
// ---------------------------------------------------------------------------
...
// 256 threads: lower 128 handle sub-block 0, upper 128 handle sub-block 1.
__global__ void cpy_tbq4_0_f32_kernel(
    const char * __restrict__ src_blocks,
    float * __restrict__ dst,
    int64_t nb
) {
    const int64_t blk = (int64_t)blockIdx.x;
    if (blk >= nb) return;
    ...
    // Step 1: unpack nibble → scaled codebook value
    {
        const uint8_t byte = qs[tid >> 1];
        const uint8_t idx  = (tid & 1) ? (byte >> 4) : (byte & 0x0F);
        smem[tid] = d_tbq4_codebook_scaled[idx];
    }
    __syncthreads();

    // Step 2: FWHT inverse on each sub-block simultaneously
    // Inverse: apply s2, butterfly passes, normalize, apply s1
    float * sub = smem + sb * 128;

    sub[lid] *= d_tbq_wht_s2[lid];
    __syncthreads();

    #pragma unroll
    for (int h = 1; h < 128; h <<= 1) {
        if ((lid & h) == 0) {
            float a = sub[lid], b = sub[lid | h];
            sub[lid]     = a + b;
            sub[lid | h] = a - b;
        }
        __syncthreads();
...
static void ggml_cpy_tbq4_0_f32_cuda(
    ...
    cpy_tbq4_0_f32_kernel<<<(unsigned int)nb, TBQ4_QK, 0, stream>>>(cx, (float *)cdst, nb);
