...
llama_kv_cache::llama_kv_cache(
    ...
    if (attn_rot_k || attn_rot_v) {
        for (int64_t n = 64; n <= (int64_t)std::max(hparams.n_embd_head_k, hparams.n_embd_head_v); n *= 2) {
...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    // TBQ types store all heads combined (block_size=256 > head_dim=128), use 3D view
...
ggml_tensor * llama_kv_cache::build_rope_shift(
    ...
    if (ggml_is_quantized(cur->type)) {
        ...
        // rotate back (inverse rotation before RoPE)
        if (rot) {
        ...
        // rotate fwd (re-apply rotation after RoPE)
        if (rot) {
            tmp = ggml_mul_mat_aux(ctx, tmp, rot);
        }
...
ggml_cgraph * llama_kv_cache::build_graph_shift(llm_graph_result * res, llama_context * lctx) const {
  ...
  //const auto & n_embd_head_v = hparams.n_embd_head_v;
    ...
    for (const auto & layer : layers) {
        ...
        ggml_tensor * cur = build_rope_shift(cparams, ctx, k, inp->k_shift, inp->k_rot, rope_factors, freq_base_l, freq_scale_l);
